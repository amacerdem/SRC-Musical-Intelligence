...
export const useGamificationStore = create<GamificationState>()(
  persist(
    (set, get) => ({
      ...
      shareMockInstagram: async (_assetUrl: string) => {
        // No-op on mobile (InstagramSimulator not available).
        // Simply grant a small resonance bonus.
        get().addResonance(5);
      },

      plantSeed: (family, track) =>
        set((state) => {
          const newCount = state.mindGarden.seedsPlanted + 1;
          const seasons: MindGardenState["season"][] = [
            "spring",
            "summer",
            "autumn",
            "winter",
          ...
          return {
            mindGarden: {
              ...
              season: nextSeason,
              recentTracks: [track, ...state.mindGarden.recentTracks].slice(
                0,
                10,
              ),
            },
