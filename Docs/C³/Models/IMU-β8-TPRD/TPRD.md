...
### 2.2 Information Flow Architecture (EAR → BRAIN → H³ direct* → TPRD)
...
| 6 | **Bidelman (2013)** | Review (brainstem FFR) | Multiple studies | Brainstem FFR encodes consonance hierarchy; subcortical pitch salience predicts perceptual consonance ratings. Hierarchical ordering well-predicted by subcortical encoding. | FFR-consonance r >= 0.81 (Bidelman & Krishnan 2009) | **Subcortical tonotopic-to-pitch transformation begins before cortex; R³→pitch-processing brainstem pathway** |
| 7 | **Patterson et al. (2002)** | fMRI | 6 | Pitch processing activates lateral HG more than medial HG; temporal regularity drives anterolateral HG selectively | — | **lateral HG responds to temporal pitch cues** |
...
| 11 | **Cheung et al. (2019)** | fMRI | 39 | Uncertainty and surprise jointly predict auditory cortex activity; interaction in amygdala, hippocampus, and auditory cortex | fMRI: interaction effect in bilateral auditory cortex, amygdala, hippocampus | **Prediction error in auditory cortex modulates pitch processing; supports pred_error role** |
...
- **H³ horizons**: H10 (400ms), H14 (700ms), H18 (2s) — harmonic syntax context
- **pitch-processing horizons**: H0 (5.8ms), H3 (23.2ms), H6 (200ms) — pitch processing chain
...
TPRD projected v2 from F (Pitch) group, aligned with corresponding H³ horizons (H10, H14, H18) and pitch-processing horizons (H0, H3, H6).
...
Pitch encoding (nonprimary HG):
 pitch = σ(0.40 · pitch_sal.mean()
 + 0.30 · R³.tonalness[14] · R³.spectral_autocorr[17])

 Coefficient sum check: |0.40| + |0.30| = 0.70 ≤ 1.0 ✓

 Rationale:
 pitch_salience: direct pitch extraction from perceptual circuit
...
Dissociation:
 ...
 pred_error: harmonic expectation violation
```
...
f32 = σ(0.40 * mean(pitch_sal[0:10]) + 0.30 * tonalness * spectral_autocorr)
...
├─────────────────────────────────────────────────────────────────────────────┤
│ │
│ pitch_salience ──────► TPRD.f32_pitch (pitch representation) │
│ consonance_encoding ─► TPRD.spectral_pitch_r (coherence) │
│ chroma_processing ───► TPRD.pitch_state (chroma abstraction) │
│ │
│ INTRA-UNIT (IMU): │
...
│ └── Shared syntactic substrate (synthesis mechanism) │
│ │
│ CROSS-UNIT (SPU): │
...
class TPRD(BaseModel):
 ...
 TIER = "β8"
 OUTPUT_DIM = 10
 CROSS_UNIT_READS = ()  # TODO: populate from Nucleus contract # Cross-circuit read from perceptual
 ...
 A_PITCH_1 = 0.40 # pitch_salience weight
 ...
 A_DISSOC_3 = 0.25 # pred_error weight sum = 0.80 ✓

 @property
 def h3_demand(self) -> List[Tuple[int, int, int, int]]:
 """18 tuples for TPRD computation."""
 return [
 # Synthesis horizons (mnemonic circuit)
 ...
 # pitch-processing horizons (cross-circuit from perceptual)
...
| **compute()** | Full implementation with H³ direct | Stub returning zeros | Code is stub only |
...
| Tonotopic encoding | S⁰.L7.crossband + S⁰.L0.freq × OSC | R³.roughness × R³.entropy × synthesis |
| Pitch extraction | S⁰.L5.centroid + S⁰.L6.tristimulus × HRM | R³.tonalness × R³.autocorr × pitch-processing* |
| Dissociation | S⁰.X_L0L5 × SGM | |f31-f32| × inharmonicity |
