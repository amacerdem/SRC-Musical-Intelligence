...
### 2.2 Information Flow Architecture (EAR → BRAIN → MTNE)
...
R³[21] spectral_change ──────────┘ short_context at H8 (300ms)
...
R³[16] roughness ─────────────────┘ medium_context at H14 (700ms)
 Math: sustain = σ(β · energy ·
 roughness · med)

R³[25:33] x_l0l5 (8D) ──────────┐
R³[33:41] x_l4l5 (8D) ──────────┼──► Long-term Efficiency
R³[0] pleasant ──────────────────┘ long_context at H20 (5000ms)
...
MTNE requires H³ features at three horizons: H8 (300ms), H14 (700ms), H20 (5000ms).
...
Minor v2 expansion. MTNE projected v2 feature from G:Rhythm, aligned with corresponding H³ horizons.
...
────┼───────────────────┼────────┼────────────────────────────────────────────
 ...
 │ │ │ medium_context + roughness features.
...
│ context_depth ↔ MTNE.efficiency_index │
...
class MTNE(BaseModel):
 ...
 ALPHA_1 = 0.30 # Inhibition: flux × onset × temporal-context weight
 ALPHA_2 = 0.25 # Inhibition: spectral std weight
 BETA_1 = 0.25 # Efficiency: energy × roughness × temporal-context weight
...
| Inhibition proxy | X_L4L5 [192:200] (Derivatives x Perceptual) | short_context + R³ flux/onset/change |
