...
where:
 ...
 V_auditory = f(R³.consonance, valence_dynamics)
...
────┼───────────────────┼────────┼────────────────────────────────────────────
 ...
 │ │ │ Loudness × affective-dynamics affect state.
────┼───────────────────┼────────┼────────────────────────────────────────────
 7 │ aud_valence_contr │[-1, 1] │ Auditory valence contribution.
 │ │ │ Consonance-weighted affect signal via cognitive-projection H³.
...
CMAT uses **H³ direct** (2 mechanisms).
...
def compute_cmat(R3, H3):
 ...
 affect_velocity = abs(affect_velocity)
 affect_entropy = affect_entropy

 integration_state = c0p_mean
 integration_var = c0p_std
 ...
 # === LAYER S ===
 # Supramodal valence (audio-only: auditory valence weighted by affect dynamics)
 supra_valence = tanh(
 ...
 # Auditory valence contribution via cognitive-projection H³
...
| Temporal | HC⁰ (9 tuples) | H³ (9 tuples) | |
...
| 2.0.0 | 2026-02-12 | MI R³/H³ architecture: +2D, H³ direct binding, R³ mapping |
