...
(Music therapist-guided)
 ...
 │ └───────────────────┘ └───────────────────┘ │
 │ │
 │ Vocal quality → pitch-processing pitch extraction │
 │ Attention → auditory-scene attention gating │
 │ Pattern memory → auditory-scene scene analysis │
 │ Prediction → auditory-scene salience weighting │
 │ │
 └─────────────────────────────────────────────────────────┘
 │
 ▼
...
### 2.1 Information Flow Architecture (EAR → BRAIN → DSP)
...
| **Edalati 2023** | EEG | 19 | Phase alignment of neural oscillations to auditory rhythm envelope | Phase-coupling at beat/meter frequencies | **pitch-processing periodicity input relevance** |
...
DSP requires H³ features for vocal pitch extraction and contour tracking, and for attentional gating during singing sessions. The demand reflects the infant auditory processing timescales.
...
Parameters:
 ...
 QualityGating = pitch-processing pitch extraction strength
```
...
| Vocal quality | S⁰.L5.spectral_centroid[38] + HC⁰.HRM | R³.consonance[4] |
| Attention | S⁰.L5.loudness[35] + HC⁰.ATT | R³.loudness[8] |
| Plasticity | S⁰.cumulative_exposure + HC⁰.OSC | R³.pitch_change[23] |
