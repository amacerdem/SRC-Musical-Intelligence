...
1. **memory-encoding mechanism (primary)**: Captures how rhythmic stimulation encodes into hippocampal memory systems, driving long-term neuroplastic change through repeated binding.

2. **beat-entrainment mechanism (cross-circuit read from sensorimotor)**: Provides the real-time beat induction, meter extraction, and motor entrainment signals that drive the memory encoding. Without beat-entrainment H³, RASN has no rhythmic input.

3. **The coupling**: encoding_state integrates beat_induction over time --- each entrainment event creates a new encoding opportunity. retrieval_dynamics tracks how the accumulated rhythmic exposure produces long-term plasticity.
...
### 2.2 Information Flow Architecture (EAR → BRAIN → H³ direct* → RASN)
...
|---|-------|--------|---|-------------|-------------|-------------|
| 1 | **Thaut, McIntosh & Hoemberg 2015** | Review | — | Rhythmic entrainment via reticulospinal pathways; beta oscillations modulated in SMA, IFG, cerebellum; IC timing for auditory-motor transformations | — (foundational) | **auditory-motor entrainment theory** |
| 2 | **Grahn & Brett 2007** | fMRI | 27 | Basal ganglia (putamen) and SMA respond preferentially to beat-inducing rhythms; metric simple > metric complex/nonmetric | F(2,50)=1.44–20.67; FDR p<.05 whole-brain | **SMA + basal ganglia = beat detection** |
| 3 | **Harrison et al. 2025** | fMRI | 55 (27 PD + 28 HC) | Both external and internal musical cues activate sensorimotor cortex, SMA, putamen; external cues additionally activate auditory cortex; internal cues additionally activate cerebellum; no PD vs. HC differences | FWE-corrected clusters | **CTC + SPT pathways for RAS** |
| 4 | **Blasi et al. 2025** | Systematic review (20 RCTs) | 718 | Music/dance interventions produce structural neuroplasticity: hippocampal volume increases, GMV changes in frontal/temporal areas, WM integrity in FAT/AF/SLF/CST | review (20 RCTs) | **structural neuroplasticity from rhythm interventions** |
| 5 | **Noboa, Kertesz & Honbolygo 2025** | EEG | 30 | SS-EPs at beat-related frequencies (1.25 Hz, harmonics 2.5/5 Hz); working memory predicts tapping consistency; stronger entrainment ≠ better synchronization | SS-EP amplitudes at 1.25 Hz | **neural tracking of beat frequency** |
| 6 | **Ding et al. 2025** | EEG | 37 | All 12 presenting rates (1-12 Hz) entrain neural oscillations; frontocentral area; ITPC eta-sq=0.14, EPS eta-sq=0.32; 6 Hz boundary for emotional effects | F(11,330)=14.10, p<.001, eta-sq=0.32 | **frequency-specific neural entrainment** |
| 7 | **Ross & Balasubramaniam 2022** | Mini review | — | Motor networks active during passive rhythm listening; covert motor entrainment supports beat perception; prediction/error correction via sensorimotor simulation | — (theoretical) | **covert motor simulation for beat** |
| 8 | **Jiao 2025** | Review | — | 40 Hz gamma entrainment enhances cognition; music therapy modulates limbic/prefrontal/reward circuits; AI-driven biofeedback enables personalized RAS | — (review) | **entrainment-driven plasticity integration** |
| 9 | **Zhao 2025** | Systematic review | 968+ | RAS promotes neuroplasticity through entrainment and sensorimotor integration; 4 meta-analyses synthesized | review (4 meta-analyses) | **repeated entrainment → plasticity** |
| 10 | **Wang 2022** | Meta-analysis | 22 studies | RAS improves walking function across stroke populations | positive (gait velocity, stride) | **beat-movement coupling** |
| 11 | **Ghai & Ghai 2019** | Systematic review | 968 patients | RAS improves gait parameters in neurological conditions | meta-analytic positive | **entrainment quality → recovery** |
...
Phase 2: NEURAL ENTRAINMENT (200ms-1s, H11/H16)
─────────────────────────────────────────────────────
...
R³[0:7] consonance group ─────► Harmonic recognition
...
- **H³ horizons**: H16 (1s), H20 (5s), H24 (36s) — memory encoding and plasticity
- **Beat entrainment horizons**: H6 (200ms), H11 (500ms), H16 (1s) — beat entrainment (cross-circuit)
...
RASN projected v2 from G (Rhythm) group, aligned with corresponding H³ horizons (H16, H20, H24) and Beat entrainment horizons (H6, H11, H16).
...
| 69 | metricality | G | 16 | M1 (mean) | L0 | Average metricality over bar for memory encoding |
...
────┼───────────────────────┼────────┼────────────────────────────────────────────
 ...
 │ │ │ Expanded: beat.mean · encoding.mean
 │ │ │.mean · complexity_optimal
 │ │ │.mean · stumpf
────┼───────────────────────┼────────┼────────────────────────────────────────────
...
f10 = σ(0.35 · mean(R³.x_l0l5[25:33]) · mean(beat_induction[0:10])
 + 0.35 · R³.spectral_flux[10] · R³.onset_strength[11] · mean(meter_extraction[10:20])

# f11: Motor Facilitation
# Coefficients: 0.40 + 0.30 + 0.30 = 1.00 ✓
f11 = σ(0.40 · mean(R³.x_l4l5[33:41]) · mean(motor_entrainment[20:30])
...
neuroplasticity_idx = σ(
 + 0.30 · mean(motor_entrainment[20:30]) · R³.stumpf_fusion[3]
)
# Coefficients: 0.35 + 0.35 + 0.30 = 1.00 ✓

# motor_recovery: Motor recovery potential
motor_recovery = σ(
 + 0.35 · mean(beat_induction[0:10]) · R³.amplitude[7]
)
...
```
Entrainment(t) = σ(w₁ · beat_induction(t) · beat_salience(t))

Plasticity(t) = Plasticity(t-1) · (1 - 1/τ_decay) + α · Entrainment(t) · encoding(t)
...
| **SMA / pre-SMA** | L: -9, 6, 60; R: 3, 6, 66 | Beat prediction, entrainment | fMRI | Grahn & Brett 2007 (N=27, Table 2) | Rhythmic entrainment hub (via H³ direct) |
...
SMA and cerebellum are primarily sensorimotor regions (STU territory). RASN accesses them via cross-circuit relay. Hippocampus and mPFC are the mnemonic circuit regions (IMU territory) that drive the plasticity component.
...
│ │ multimodal rehabilitation model (cross-circuit read) │
│ │ │
...
class RASN(BaseModel):
 ...
 CROSS_UNIT_READS = ()  # TODO: populate from Nucleus contract # Cross-circuit read (sensorimotor)
...
| Beat detection | S⁰.L5.spectral_flux[45] × HC⁰.NPL | R³.spectral_flux[10] |
| Motor coupling | S⁰.X_L4L5[192:200] × HC⁰.GRV | R³.x_l4l5[33:41] |
| Tempo tracking | S⁰.L4.velocity_T[15] × HC⁰.PTM | R³.onset_strength[11] |
| Plasticity signal | S⁰.L9.entropy_T[116] × HC⁰.EFC | R³.entropy[23] |
