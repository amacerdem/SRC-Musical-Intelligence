...
export function FloatingNav() {
  ...
  const itemHeight = isDesktop ? 44 : 44; // min 44px for touch targets
    ...
    <>
      ...
      >
        <div className={isMobile
          ? "flex items-center justify-around bg-black/90 backdrop-blur-xl border-t border-white/[0.10] px-2 py-1.5"
          : `nav-dock flex items-center px-2.5 py-2 gap-1`
        }>
          {mainNav.map(({ to, icon: Icon, labelKey, belief }) => {
            ...
            return (
              <NavLink key={to} to={to} className={isMobile ? "relative group flex-1" : "relative group"}>
                <div
                  className={isMobile
                    ? "flex flex-col items-center justify-center gap-0.5 rounded-xl transition-all duration-300"
                    : "flex items-center gap-1.5 rounded-full px-3 transition-all duration-300"
                  }
                  style={{
                    height: itemHeight,
                    background: isActive ? `${accentColor}15` : "transparent",
                    border: isMobile ? "none" : isActive ? `1px solid ${accentColor}25` : "1px solid transparent",
                    boxShadow: isActive && !isMobile ? `0 0 20px ${accentColor}10, inset 0 0 12px ${accentColor}05` : "none",
                  }}
                >
                  <Icon
                    size={isMobile ? 20 : iconSize}
                    className={`transition-all duration-300 ${isActive ? "" : "text-slate-500 group-hover:text-slate-200"}`}
                    style={{
                  ...
                  />
                  <span
                    className={`font-display font-medium transition-all duration-300 ${isMobile ? "text-[9px]" : isDesktop ? "text-[11px]" : "text-[10px]"} ${isActive ? "" : "text-slate-500 group-hover:text-slate-200"}`}
                    style={{
                      color: isActive ? accentColor : undefined,
      ...
      <motion.div
        ...
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.8, delay: 0.5 }}
        className="fixed top-5 right-4 md:right-6 z-40 flex items-center gap-2 md:gap-3"
        style={{ top: "calc(env(safe-area-inset-top, 0px) + 20px)" }}
      >
        <LanguageToggle />
        ...
        <button
          ...
          className="w-10 h-10 rounded-full flex items-center justify-center text-slate-600 hover:text-slate-300 transition-colors duration-300"
        >
          <Search size={14} />
        </button>
        ...
        {persona && (
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-mono text-slate-600">L{personaLevel}/12</span>
            <div className="relative">
              <div
        ...
        >
          <div className="glass p-1 w-72 lg:w-80">
            <input
              ...
              className="w-full px-4 py-2.5 bg-transparent text-sm text-slate-200 placeholder:text-slate-600 outline-none font-body"
            />
          </div>
        </motion.div>
      )}
