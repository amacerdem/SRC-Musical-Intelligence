...
College, Trinity Institute of Neuroscience, The University of Dublin, Dublin, Ireland, 4 Centre for Biomedical Engineering,
School of Electrical and Electronic Engineering and UCD University College Dublin, Dublin, Ireland, 5 Institute for Systems
...
brain-computer interface solutions.
Keywords: EEG, cortical, TRF, neural tracking, pitch, music, signal processing, decoding
INTRODUCTION
In our everyday life, our brain examines sounds by extracting various types of auditory features.
In music, one such feature is the melody, which is a sequence of pitches set to a particular rhythm
...
Di Liberto et al.
Decoding Imagined Melodies From Electroencephalography
...
The ability to measure this multifaceted neural encoding of
...
et al., 2018; Obleser and Kayser, 2019; Di Liberto et al., 2021b).
While that work informed us on which properties of music are
...
encoded in the neural signal and contribute to the decoding.
Furthermore, the development of such a classiﬁer for the case of
music imagery could directly translate into new brain-computer
interfaces (BCIs).
Recent work demonstrated that EEG signals recorded during
...
carried out at École Normale Supérieure, in a dimmed room.
Participants were instructed to minimise motor activities while
performing the task.
The experiment consisted of 88 trials in which participants
...
correction was applied for all participants with the same lag value.
Before the experiment, musical imagery skills (or audiation skills)
were assessed for every subject with “The Advanced Measures of
Music Audiation” test1 (AMMA).
Stimuli and Procedure
Four melodies were selected from a monophonic MIDI corpus
of Bach chorales (BWV 349, BWV 291, BWV354, BWV 271).
Each melody was repeated 11 times per condition and the
...
tenor, and bass) accompanying the initial melody on soprano.
The monophonic version of those melodies consist of the
...
then re-referenced to the average of the 64 channels.
...
available at http://audition.ens.fr/adc/NoiseTools/).
This technique allows the extraction of a “consensus signal”
...
Joint Decorrelation Analysis (JD)
...
Sound Envelope Reconstruction Analysis
(bTRF)
...
space to the univariate stimulus feature space (backward model).
Here, we used an implementation of such backward modelling
...
The interaction between stimulus and recorded brain
...
2019; Di Liberto et al., 2020, 2021a; Marion et al., 2021).
In order to decode the melody, stimulus and EEG data were
...
was always a multiple of the duration of a music bar (2.4 s).
One strength of this decoding approach is that it operates in
...
decoding short segments of music.
Maximum Correlation Music Decoding
(maxCorr)
...
had larger variance over the entire duration of the experiment.
The reference and test sets of melody segments were built by
...
segment sizes, always multiples of the length of a music bar.
Note that longer segments would lead to smaller reference sets
...
(for the longest segment length).
Evaluation Metrics
Two distinct evaluation metrics were devised to compare the
decoding results for bTRFenv and maxCorr (Figure 1C). The ﬁrst
metric evaluated the accuracy in the decoding of note-onsets.
Such a value was determined as the ratio between the number
...
information beyond the note timing.
...
by means of permutation tests with N = 100 shuﬄes.
RESULTS
...
envelope based linear decoding models.
The use of two distinct decoding metrics (note-onset
...
set across all segment durations (see section “Materials and Methods”).
Supplementary Figure 1 shows the results of this analysis for EEG ﬁltered in
...
segment, thus to the precise identiﬁcation of pitch progressions.
To assess whether pitch-related EEG signals contributed to the
...
EEG information to the decoding (Tukey’s HSD, p < 0.05).
As expected, such a contribution did not emerge for longer
segment durations, which was an expected outcome because
timing information is suﬃcient to uniquely characterise long
segments of music.
Similar but overall weaker eﬀects were measured when
...
p = 5.3 × 10−11]; and condition [imagery vs. listening;
F(1, 20) = 74.0, p = 3.7 × 10−8]. Signiﬁcant interactions
...
condition [F(3, 60) = 24.0, p = 1.6 × 10−7].
Signiﬁcant melody decoding were also measured at the
...
21 participants.
The individual-subject result was then compared with the
...
Further analyses were conducted to assess which EEG
...
FIGURE 4 | Robust melody decoding relies on low-frequency EEG (<1 Hz).
Melody decoding scores obtained with maxCorr are compared for EEG ﬁltered
...
no signiﬁcant eﬀect of condition [F(1, 20) = 2.7, p = 0.12].
Signiﬁcant interactions were measured between frequency-band
...
imagery condition.
The use of a broader EEG frequency-band provides us with
...
analysis improves the results for imagery but not for listening.
Speciﬁcally, a three-way repeated measures ANOVA on the
...
in this section.
Recent research on speech perception provided substantial
...
neural encoding also occurs in the case of auditory imagery.
In line with our recent ﬁnding that both music listening and
...
in the low-frequency neural signals below 1 Hz (Figure 4).
Interestingly, such slow rhythms below 1 Hz did not contribute
...
the bTRFenv.
One open issue is determining what speciﬁc music properties
are encoded in the EEG signal and contribute to the decoding.
Indeed, a robust EEG signature of absolute or relative pitch
...
melodic context), which act here as proxy indices for pitch.
Indeed, EEG has been shown to encode other pitch-related
...
The neural encoding of music was previously shown to
...
such as encoding models, is a valid and feasible approach.
Indeed, this result is highly dependent on the use of an
...
eliciting a consistent neural activation at each occurrence.
Extensive work and discussions have been conducted on
...
decoding of melodies.
Our results show that accurate decoding of melodies
...
Our ﬁnding is particularly relevant for BCI applications as
...
be possible in the presence of a rich reference EEG set.
In this regard, further work is needed to determine optimal
...
to utilise this experimental paradigm on non-musicians too.
While we speculate that our results apply to the general
...
involving imagery during silent gaps in familiar music (e.g.,
Gabriel et al., 2016).
...
maxCorr baseline after shufﬂing the pitch values (grey solid lines).
Supplementary Figure 2 | Single-subject Melody Decoding (1–30 Hz EEG).
Single-subject melody decoding from EEG (1–30 Hz) with maxCorr. Note-onset
...
indices of the EEG segments for each subject).
Supplementary Figure 3 | Linear decoding models (bTRFenv) fail to capture
...
conditions (three-way repeated measures ANOVA, main effect of frequency-band;
Note-onset: p = 0.09; pitch: p = 4.4 × 10−3, post hoc Tukey’s HSD; *p < 0.05,
**p < 0.01, ***p < 0.001).
REFERENCES
...
3976–3983.e5.
Carrus, E., Pearce, M. T., and Bhattacharya, J. (2013). Melodic pitch expectation
...
de Cheveigné, A., Di Liberto, G. M., Arzounian, D., Wong, D., Hjortkjaer, J.,
Fuglsang, S. A., et al. (2019). Multiway canonical correlation analysis of brain
...
de Cheveigné, A., and Simon, J. Z. (2008). Denoising based on spatial ﬁltering.
J. Neurosci. Methods 171, 331–339. doi: 10.1016/j.jneumeth.2008.03.015
...
Di Liberto, G. M., Marion, G., and Shamma, S. A. (2021a). The music of silence.
Part II: musical listening induces imagery responses. J. Neurosci, JN-RM-0184-
...
88, 41–46.
Freitas, C., Manzato, E., Burini, A., Taylor, M. J., Lerch, J. P., and Anagnostou, E.
...
spontaneously evoked by gaps of silence in familiar and newly learned songs.
Neurobiol. Learn. Mem. 132, 18–28. doi: 10.1016/j.nlm.2016.04.011
...
responses to familiar vs. unfamiliar music – an EEG and Pupillometry study.
Sci. Rep. 9:15570.
...
temporal processing properties of the auditory system using continuous stimuli.
J. Neurophysiol. 102, 349–359. doi: 10.1152/jn.90896.2008
Margulis, E. H. (2014). On Repeat. Oxford: Oxford University Press. doi: 10.1093/
acprof:oso/9780199990825.001.0001
Marion, G., Di Liberto, G. M., and Shamma, S. A. (2021). The music of silence.
Part I: responses to musical imagery accurately encode melodic expectations
...
the Listening Brain. Trends Cogn. Sci. 23, 913–926. doi: 10.1016/j.tics.2019.08.
004
Omigie, D., Pearce, M. T., Williamson, V. J., and Stewart, L. (2013).
Electrophysiological correlates of melodic processing in congenital amusia.
Neuropsychologia 51, 1749–1762. doi: 10.1016/j.neuropsychologia.2013.05.010
Patel, A. D. (2003). “A New Approach to the Cognitive Neuroscience of Melody,”
in The Cognitive Neuroscience of Music, eds I. Peretz and R. Zatorre (Oxford:
Oxford University Press). doi: 10.1093/acprof:oso/9780198525202.003.0021
Frontiers in Neuroscience | www.frontiersin.org
10
August 2021 | Volume 15 | Article 673401
...
the dynamic representation of musical pitch from human brain activity. Sci.
Rep. 8:839.
Tal, I., Large, E. W., Rabinovitch, E., Wei, Y., Schroeder, C. E., Poeppel, D.,
et al. (2017). Neural entrainment to the beat: the “missing-pulse” phenomenon.
J. Neurosci. 37, 6331–6341. doi: 10.1523/JNEUROSCI.2500-16.2017
...
Copyright © 2021 Di Liberto, Marion and Shamma. This is an open-access article
