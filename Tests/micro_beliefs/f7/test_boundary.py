"""Boundary condition tests — F7 extreme inputs.

Tests that all 17 F7 beliefs produce valid output for edge cases:
  - Near-silence (floor)
...
class TestBoundaryNearSilence:
    ...
    def test_all_beliefs_valid(self, runner):
        ...
        results = runner.run(audio, F7_ALL_BELIEFS)
...
class TestBoundaryLongDuration:
    """Long duration (~45s): all beliefs valid, no accumulation errors."""

    def test_all_beliefs_valid(self, runner):
        audio = _load("boundary", "08_long_duration_45s")
        results = runner.run(audio, F7_ALL_BELIEFS)
        for b in F7_ALL_BELIEFS:
