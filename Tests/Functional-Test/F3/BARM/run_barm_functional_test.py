"""BARM Functional Test — v1.0
Tests BARM 10D: E0-E2, M0-M1, P0-P1, F0-F2.
Brainstem Auditory Response Modulation. Encoder depth 1, reads SNEM.
"""
...
DIM_NAMES = ["E0:regularization","E1:beat_alignment","E2:sync_benefit",
    "M0:veridical_percept","M1:regularization_eff","P0:beat_accuracy","P1:regularization_str",
    "F0:beat_acc_pred","F1:sync_benefit_pred","F2:individual_diff"]
OUTPUT_DIM = 10
...
class BARMTestRunner:
    ...
    def _load_and_run(self, name):
        ...
        relay = outputs.get("BARM")
        if relay is None: raise RuntimeError(f"BARM not found for '{name}'")
    ...
    def test_T4_beat(self):
        G="T4"
        for s in ["g1_01_single","g4_02_melody","g4_03_arpeggio"]:
            e1=self._mean(s,E1); self._test(G,f"T4_E1_pos_{s}",e1>0.0,f"E1({e1:.4f})>0")
    def test_T5_m_layer(self):
        ...
        self._test(G,"T5_M0_E1_corr",abs(r)>0.10,f"|r(M0,E1)|={abs(r):.4f}>0.10 (E1 feeds M0 at 50%)")
    ...
    def test_T7_forecast(self):
        ...
        self._test(G,"T7_F0_P0_corr",abs(r)>0.10,f"|r(F0,P0)|={abs(r):.4f}>0.10 (P0 feeds F0 at 50%)")
        p0s2=np.array([self._mean(s,P0) for s in stims]); f2s=np.array([self._mean(s,F2) for s in stims])
        r2=float(np.corrcoef(p0s2,f2s)[0,1])
        self._test(G,"T7_F2_P0_corr",abs(r2)>0.10,f"|r(F2,P0)|={abs(r2):.4f}>0.10 (P0 feeds F2 at 50%)")
    ...
    def test_T9_redundancy(self):
        G="T9"
        COUPLED={
            (0,4),(1,3),(2,3),(1,5),(0,5),(0,6),(3,5),(4,6),
            (1,7),(2,8),(5,7),(5,9),(6,7),(6,9),(7,9),
            (0,7),(1,9),(3,7),(3,9),(4,5),
        }
    ...
    def run_all(self):
        ...
        for t in [self.test_T1_dimensionality,self.test_T2_bounds,self.test_T3_nondegeneracy,
                  self.test_T4_beat,self.test_T5_m_layer,self.test_T6_p_layer,
    ...
    def _report(self, elapsed):
        ...
        out=RESULTS_DIR/f"barm_results_{ts}.json"
        with open(out,"w") as f: json.dump({"mechanism":"BARM","function":"F3","timestamp":ts,"total":total,"passed":passed,"failed":failed,"elapsed_s":round(elapsed,1),"results":[{"name":r.name,"group":r.group,"passed":r.passed,"message":r.message,"values":r.values} for r in self.results]},f,indent=2)
        print(f"\n  Report: {out}")

if __name__=="__main__": BARMTestRunner().run_all()
