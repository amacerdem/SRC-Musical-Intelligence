"""STANM Functional Test — v1.0
Tests STANM 11D: E0-E2, M0-M2, P0-P1, F0-F2.
Spectrotemporal Attention Network Model. Encoder depth 1.
NOTE: M2 (lateralization) and F1 (lateral_pred) use TANH → [-1,1] bounds.
"""
...
E0=0;E1=1;E2=2;M0=3;M1=4;M2=5;P0=6;P1=7;F0=8;F1=9;F2=10
DIM_NAMES = ["E0:temporal_attn","E1:spectral_attn","E2:network_topology",
    "M0:network_topo_m","M1:local_clustering","M2:lateralization",
    "P0:temporal_alloc","P1:spectral_alloc","F0:network_state_pred","F1:lateral_pred","F2:compensation_pred"]
OUTPUT_DIM = 11
...
class STANMTestRunner:
    ...
    def _load_and_run(self, name):
        ...
        relay = outputs.get("STANM")
        if relay is None: raise RuntimeError(f"STANM not found for '{name}'")
    ...
    def test_T2_bounds(self):
        """M2 and F1 use TANH → [-1,1]. All others → [0,1]."""
        G="T2"
        TANH_DIMS = {M2, F1}
    ...
    def test_T4_attention(self):
        G="T4"
        for s in ["g1_01_single","g4_02_melody","g4_03_arpeggio"]:
            e0=self._mean(s,E0); e1=self._mean(s,E1)
            self._test(G,f"T4_E0_{s}",e0>0.0,f"E0({e0:.4f})>0")
            self._test(G,f"T4_E1_{s}",e1>0.0,f"E1({e1:.4f})>0")
    def test_T5_lateralization(self):
        """M2 = tanh(0.50×(E1-E0)). F1 = M2 (pass-through). Should be in [-1,1]."""
        G="T5"
        for s in ["g1_01_single","g4_02_melody","g3_04_dense"]:
            m2=self._mean(s,M2); f1=self._mean(s,F1)
            self._test(G,f"T5_M2_tanh_{s}",-1.0<=m2<=1.0,f"M2({m2:.4f})∈[-1,1]")
            self._test(G,f"T5_F1_eq_M2_{s}",abs(m2-f1)<0.05,f"|M2-F1|={abs(m2-f1):.4f}<0.05 (pass-through)")
    def test_T6_m_layer(self):
        G="T6"
        stims=["g1_01_single","g1_05_minor_2nd","g3_04_dense","g4_01_sustained","g4_03_arpeggio"]
        e0s=np.array([self._mean(s,E0) for s in stims]); m0s=np.array([self._mean(s,M0) for s in stims])
        r=float(np.corrcoef(e0s,m0s)[0,1])
        self._test(G,"T6_M0_E0_corr",abs(r)>0.10,f"|r(M0,E0)|={abs(r):.4f}>0.10 (E0 feeds M0 at 40%)")
    def test_T7_p_layer(self):
        ...
        for s in ["g1_01_single","g1_05_minor_2nd","g3_04_dense","g4_01_sustained","g4_03_arpeggio"]:
            for d,nm in [(P0,"P0"),(P1,"P1")]:
                v=self._mean(s,d); self._test(G,f"T7_{nm}_{s}",v>0.0,f"{nm}({v:.4f})>0")
    def test_T8_forecast(self):
        ...
        r=float(np.corrcoef(m1s,f0s)[0,1])
        self._test(G,"T8_F0_M1_corr",abs(r)>0.10,f"|r(F0,M1)|={abs(r):.4f}>0.10 (M1 feeds F0 at 50%)")
    def test_T9_instrument(self):
        ...
        for d in range(OUTPUT_DIM):
            ...
            if d in TANH_DIMS:
                self._test(G,f"T9_{DIM_NAMES[d]}",-1.0<=p<=1.0 and -1.0<=o<=1.0,f"piano={p:.4f},organ={o:.4f}∈[-1,1]")
            else:
                self._test(G,f"T9_{DIM_NAMES[d]}",p>0.0 and o>0.0,f"piano={p:.4f},organ={o:.4f}>0")
    def test_T10_redundancy(self):
        G="T10"
        COUPLED={
            (0,3),(1,3),(0,6),(1,7),(2,4),(3,6),(3,7),(0,5),(1,5),
            (5,9),(4,8),(4,10),(8,10),
            (0,7),(1,6),(2,8),(2,10),(6,7),
        }
        ...
        for i in range(OUTPUT_DIM):
            for j in range(i+1,OUTPUT_DIM):
                ...
                self._test(G,f"T10_{i}_{j}",r<0.99,f"|r({DIM_NAMES[i]},{DIM_NAMES[j]})|={r:.3f}<0.99")
    def run_all(self):
        t0=time.time(); self._init_pipeline()
        for t in [self.test_T1_dimensionality,self.test_T2_bounds,self.test_T3_nondegeneracy,
                  self.test_T4_attention,self.test_T5_lateralization,self.test_T6_m_layer,
                  self.test_T7_p_layer,self.test_T8_forecast,self.test_T9_instrument,self.test_T10_redundancy]:
    ...
    def _report(self, elapsed):
        ...
        out=RESULTS_DIR/f"stanm_results_{ts}.json"
        with open(out,"w") as f: json.dump({"mechanism":"STANM","function":"F3","timestamp":ts,"total":total,"passed":passed,"failed":failed,"elapsed_s":round(elapsed,1),"results":[{"name":r.name,"group":r.group,"passed":r.passed,"message":r.message,"values":r.values} for r in self.results]},f,indent=2)
        print(f"\n  Report: {out}")

if __name__=="__main__": STANMTestRunner().run_all()
