"""AACM Functional Test — v1.0
Tests AACM 10D: E0-E2, M0-M1, P0-P1, F0-F2.
Aesthetic-Attention Coupling Model. Encoder depth 1, reads CSG.
"""
...
E0=0; E1=1; E2=2; M0=3; M1=4; P0=5; P1=6; F0=7; F1=8; F2=9
DIM_NAMES = ["E0:attentional_engage","E1:motor_inhibition","E2:savoring_effect",
    "M0:aesthetic_engage","M1:rt_appreciation","P0:n1p2_engage","P1:aesthetic_judgment",
    "F0:behavioral_pred","F1:n2p3_pred","F2:aesthetic_pred"]
OUTPUT_DIM = 10
...
class AACMTestRunner:
    ...
    def _load_and_run(self, name):
        ...
        relay = outputs.get("AACM")
        if relay is None: raise RuntimeError(f"AACM not found for '{name}'")
    ...
    def test_T1_dimensionality(self):
        G="T1_dimensionality"; r=self._load_and_run("g1_01_single")
    ...
    def test_T4_aesthetic(self):
        G="T4_aesthetic"
        for s in ["g1_01_single","g1_03_fifth","g4_02_melody"]:
            e0=self._mean(s,E0); self._test(G,f"T4_E0_pos_{s}",e0>0.0,f"E0({e0:.4f})>0")
    def test_T5_savoring(self):
        G="T5_savoring"
        for s in ["g1_01_single","g4_01_sustained","g4_03_arpeggio"]:
            e2=self._mean(s,E2); self._test(G,f"T5_E2_bounded_{s}",0.0<=e2<=1.0,f"E2({e2:.4f})")
    def test_T6_m_layer(self):
        ...
        self._test(G,"T6_M0_E0_corr",abs(r)>0.10,f"|r(M0,E0)|={abs(r):.4f}>0.10 (50%)")
    ...
    def test_T8_forecast(self):
        ...
        r=float(np.corrcoef(e2s,f0s)[0,1])
        self._test(G,"T8_F0_E2_corr",abs(r)>0.10,f"|r(F0,E2)|={abs(r):.4f}>0.10 (E2 feeds F0 at 50%)")
    ...
    def test_T10_redundancy(self):
        G="T10_redundancy"
        COUPLED={(0,3),(0,5),(0,9),(1,8),(2,4),(2,7),(2,9),(3,5),(3,9),(4,6),(4,7),(5,8),(0,4),(0,7),(1,5),(2,6)}
    ...
    def run_all(self):
        ...
        for t in [self.test_T1_dimensionality,self.test_T2_bounds,self.test_T3_nondegeneracy,
                  self.test_T4_aesthetic,self.test_T5_savoring,self.test_T6_m_layer,
    ...
    def _report(self, elapsed):
        ...
        report={"mechanism":"AACM","function":"F3","timestamp":ts,"total":total,"passed":passed,"failed":failed,
                "elapsed_s":round(elapsed,1),"results":[{"name":r.name,"group":r.group,"passed":r.passed,"message":r.message,"values":r.values} for r in self.results]}
        out=RESULTS_DIR/f"aacm_results_{ts}.json"
        with open(out,"w") as f: json.dump(report,f,indent=2)
        print(f"\n  Report: {out}")

if __name__=="__main__": AACMTestRunner().run_all()
