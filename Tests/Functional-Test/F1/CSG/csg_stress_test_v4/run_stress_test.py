...
def stage_4_boundary_analysis(
    ...
    # CSG max H³ horizon = H16 = 172 frames ≈ 1s.  Edge regions within the
    # warm-up zone are expected to differ from the interior — this is inherent
    # to causal temporal processing, not a processing bug.
    #
    # Stage 4 checks for GENUINE processing artifacts:
    #   (a) NaN / Inf in any frame
    #   (b) Saturation — all edge frames stuck at activation bounds
    #   (c) Frame-to-frame discontinuity at the H³ warm-up boundary
    #   (d) Dead signal — zero variance across entire trace
    #
    # Mean-deviation statistics are reported as INFO (not PASS/FAIL).
    ...
    report: Dict[str, Any] = {
        "stage": "4_boundary",
        "timestamp": datetime.now().isoformat(),
        "h3_warmup_frames": H3_WARMUP,
        "edge_frames_analyzed": EDGE_FRAMES,
        "checks": ["nan_inf", "saturation", "discontinuity", "dead_signal"],
        "per_audio": {},
        "summary": {},
    }
    ...
    for audio_key, result in results.items():
        ...
        for d_idx, d_name in enumerate(CSG_DIMENSIONS):
            ...
            # --- (a) NaN/Inf check ---
            has_nan = bool(np.any(np.isnan(trace)))
            has_inf = bool(np.any(np.isinf(trace)))

            # --- (b) Saturation check (first/last 8 frames all at bounds) ---
            first_edge = trace[:EDGE_FRAMES]
            last_edge = trace[-EDGE_FRAMES:]
            # Determine expected bounds from dimension activation
            is_tanh = d_name in ("E2:consonance_valence", "P1:affective_evaluation", "F0:valence_pred")
            lo_bound, hi_bound = (-1.0, 1.0) if is_tanh else (0.0, 1.0)
            first_saturated = bool(np.all(np.abs(first_edge - lo_bound) < 1e-6) or np.all(np.abs(first_edge - hi_bound) < 1e-6))
            last_saturated = bool(np.all(np.abs(last_edge - lo_bound) < 1e-6) or np.all(np.abs(last_edge - hi_bound) < 1e-6))

            # --- (c) Discontinuity at H³ warm-up boundary ---
            ...
            is_dead = trace_std < 1e-8

            # --- Edge statistics (informational) ---
            interior_start = min(H3_WARMUP, T // 4)
            interior_end = max(T - H3_WARMUP, T * 3 // 4)
            if interior_end > interior_start + EDGE_FRAMES:
                interior = trace[interior_start:interior_end]
                interior_mean = float(np.mean(interior))
                interior_range = max(float(np.ptp(interior)), 0.01)
                first_dev_pct = abs(float(np.mean(first_edge)) - interior_mean) / interior_range * 100
                last_dev_pct = abs(float(np.mean(last_edge)) - interior_mean) / interior_range * 100
            else:
                first_dev_pct = last_dev_pct = 0.0
                interior_mean = float(np.mean(trace))
                interior_range = float(np.ptp(trace))

            # Determine artifact status
            is_artifact = has_nan or has_inf or first_saturated or last_saturated or has_discontinuity or is_dead
            if is_artifact:
            ...
            dim_report = {
                "nan": has_nan,
                "inf": has_inf,
                "first_saturated": first_saturated,
                "last_saturated": last_saturated,
                "warmup_jump": round(warmup_jump, 6),
                "warmup_jump_last": round(warmup_jump_last, 6),
                "discontinuity": has_discontinuity,
                "dead_signal": is_dead,
                "trace_std": round(trace_std, 6),
                "artifact": is_artifact,
                "info": {
                    "interior_mean": round(interior_mean, 6),
                    "interior_range": round(interior_range, 6),
                    "first_edge_dev_pct": round(first_dev_pct, 2),
                    "last_edge_dev_pct": round(last_dev_pct, 2),
                },
            }
        ...
        if audio_report["artifacts_found"] == 0:
            ...
            print(f"  [{audio_key}] FAIL ({audio_report['artifacts_found']} artifacts)")
            for d_name, d_rep in audio_report["dimensions"].items():
                if not d_rep["artifact"]:
                    continue
                reasons = []
                if d_rep["nan"]: reasons.append("NaN")
                if d_rep["inf"]: reasons.append("Inf")
                if d_rep["first_saturated"]: reasons.append("first_saturated")
                if d_rep["last_saturated"]: reasons.append("last_saturated")
                if d_rep["discontinuity"]: reasons.append(f"discontinuity={d_rep['warmup_jump']:.3f}")
                if d_rep["dead_signal"]: reasons.append("dead_signal")
                print(f"     {d_name}: {', '.join(reasons)}")
...
def stage_5_comparative_report(
    ...
    for midi_key in sorted(midi_keys):
        ...
        # Exclude H³ warm-up zone for analysis
        warmup = min(180, T // 4)  # 180 frames or 25% of signal
        stable = csg[warmup:-warmup] if T > warmup * 2 else csg
        T_stable = stable.shape[0]

        # Dimension means (warm-up excluded)
        dim_means: Dict[str, float] = {}
        for d_idx, d_name in enumerate(CSG_DIMENSIONS):
            if d_idx < stable.shape[1]:
                dim_means[d_name] = float(np.mean(stable[:, d_idx]))
        analysis["dim_means"] = {k: round(v, 4) for k, v in dim_means.items()}

        # Temporal pattern check (first half vs second half, warm-up excluded)
        half = T_stable // 2
        if half > 0:
            first_half = {d: float(np.mean(stable[:half, i])) for i, d in enumerate(CSG_DIMENSIONS) if i < stable.shape[1]}
            second_half = {d: float(np.mean(stable[half:, i])) for i, d in enumerate(CSG_DIMENSIONS) if i < stable.shape[1]}
            ...
            if expectations.get("temporal_pattern") == "decrease":
                ...
                # Exclude H³ warm-up zone (first/last 180 frames) for flatness check
                warmup = min(180, T // 4)
                e0_stable = csg[warmup:-warmup, 0] if T > warmup * 2 else csg[:, 0]
                e0_std = float(np.std(e0_stable))
    ...
    for hk in hierarchy_keys:
        if hk in results:
            ...
            if csg is not None and csg.shape[1] > 2:
                # Exclude H³ warm-up zone
                w = min(180, csg.shape[0] // 4)
                e2_stable = csg[w:-w, 2] if csg.shape[0] > w * 2 else csg[:, 2]
                e2_values[hk] = float(np.mean(e2_stable))
