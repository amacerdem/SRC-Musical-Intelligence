"""Generate PCCR test stimuli — 16 MIDI audio files.

Targets PCCR's chroma/pitch-class representations with:

  1. Consonance hierarchy: single → minor 2nd (6 files)
  2. Complexity: single → dense cluster (4 files)
  3. Register: C3 → C6 (3 files) — octave equivalence test
  4. Temporal: sustained → arpeggio (3 files)

All files rendered via FluidSynth + SoundFont, saved to
Tests/Functional-Test/F1/PCCR/stimuli/
"""
...
_save("g2_01_single", midi_note(C4, DUR), {"group": "complexity", "pattern": "single"})
_save("g2_02_dyad", midi_chord([C4, G4], DUR), {"group": "complexity", "pattern": "dyad"})
_save("g2_03_triad", midi_chord([C4, E4, G4], DUR), {"group": "complexity", "pattern": "triad"})
_save("g2_04_dense", midi_chord([C4, Db4, D4, Eb4], DUR), {"group": "complexity", "pattern": "dense"})

# GROUP 3: Register (octave equivalence)
print("GROUP 3: Register")
_save("g3_01_low", midi_note(C3, DUR), {"group": "register", "notes": "C3"})
_save("g3_02_mid", midi_note(C4, DUR), {"group": "register", "notes": "C4"})
_save("g3_03_high", midi_note(C6, DUR), {"group": "register", "notes": "C6"})
