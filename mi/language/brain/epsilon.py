"""
ε (Epsilon) — Learning Dynamics (19D)

Level 5: HOW the listener learns from the music over time.
STATEFUL: maintains online statistics across frames.

Surprise & Entropy (2D):
  ε0: surprise — transition surprisal                     [Pearce 2005 IDyOM]
  ε1: entropy — state uncertainty                         [Shannon 1948]

Prediction Errors (3D):
  ε2: pe_short — short-term prediction error (~58ms)      [Koelsch 2019]
  ε3: pe_medium — medium-term prediction error (~580ms)   [Koelsch 2019]
  ε4: pe_long — long-term prediction error (~5.8s)        [Koelsch 2019]

Precision (2D):
  ε5: precision_short — short-term confidence             [Friston 2010]
  ε6: precision_long — long-term confidence               [Friston 2010]

Information Dynamics (3D):
  ε7: bayesian_surprise — belief update magnitude         [Itti & Baldi 2009]
  ε8: information_rate — mutual information past→present  [Dubnov 2008]
  ε9: compression_progress — learning rate as reward      [Schmidhuber 2009]

Interaction (1D):
  ε10: entropy_x_surprise — pleasure predictor            [Cheung et al. 2019]

ITPRA (5D):
  ε11: imagination — long-term pleasure baseline          [Huron 2006 ITPRA-I]
  ε12: tension_uncertainty — entropy as tension           [Huron 2006 ITPRA-T]
  ε13: prediction_reward — reward for correct prediction  [Huron 2006 ITPRA-P]
  ε14: reaction_magnitude — surprise magnitude            [Huron 2006 ITPRA-R]
  ε15: appraisal_learning — compression progress          [Huron 2006 ITPRA-A]

Reward & Aesthetics (3D):
  ε16: reward_pe — reward prediction error                [Gold et al. 2019]
  ε17: wundt_position — inverted-U optimal arousal        [Berlyne 1971]
  ε18: familiarity — exposure accumulation                [Zajonc 1968]
"""
...
class EpsilonGroup(BaseSemanticGroup):
    ...
    def compute(self, brain_output: object, **kwargs) -> SemanticGroupOutput:
        """Compute ε group with online state accumulation.

        Tracks pleasure (D9) and arousal (D0) from BrainOutput.
        """
        tensor_26 = brain_output.tensor  # (B, T, 26)
        B, T, _ = tensor_26.shape
        device = tensor_26.device
        dtype = tensor_26.dtype

        if not self._state_initialized or self._batch_size != B:
            self._init_state(B, device, dtype)

        # Extract tracked features DIRECTLY from Brain
        pleasure = brain_output.get_dim("pleasure")   # (B, T)
        arousal = brain_output.get_dim("arousal")      # (B, T)
        ...
        for t in range(T):
            ...
            self._welford_mean = (
                self._welford_mean + delta_w / self._welford_count
            )
            ...
            entropy = -(tp_row * torch.log2(tp_row + self.EPS)).sum(
                dim=-1
            ) / log2_n
            ...
            self._buffer_count = min(
                self._buffer_count + 1, self.BUFFER_SIZE
            )
            ...
            tension_unc = entropy
            pred_reward = torch.exp(
                -abs_pe_m / (sigma_m + self.EPS)
            )
