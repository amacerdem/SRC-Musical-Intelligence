"""
MI Core Constants — SINGLE SOURCE OF TRUTH

All numerical constants for the Musical Intelligence system.
...
R3_DIM: int = 49  # Total R³ features per frame
...
MORPH_NAMES: tuple[str, ...] = (
    ...
    "velocity",          # M8:  First derivative (dR³/dt)
...
# Calibrated against Swan Lake H³ signal ranges (Feb 2026).
...
def h3_flat_index(horizon: int, morph: int, law: int) -> int:
    """Flat index into 2304D H³ space: h*72 + m*3 + l."""
    return horizon * (N_MORPHS * N_LAWS) + morph * N_LAWS + law


def scale_h3_value(value: "Tensor", morph: int) -> "Tensor":
    """Scale a single H³ value by its morph's gain and bias.

    Args:
        value: (B, T) raw H³ morph value
