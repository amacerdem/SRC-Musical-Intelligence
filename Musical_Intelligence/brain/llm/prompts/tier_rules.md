# Musical Mind — Tier Rules

## Subscription Tiers

### Free — Surface (6D Psychology)
...
### Research — Full (131D C³ Internals)
- **Sees**: All 131 beliefs + 24D + 12D + 6D
- **Language**: System internals — belief indices, relay names, kernel phases
- **Observations**: + musical_counseling (L9+), cross_m3_insight (L9+), meta_awareness (L11+)
- **Max depth**: "belief[78] wanting_ramp = 0.84 (DAED relay, F6). Bayesian gain = π_obs/(π_obs+π_pred) = 0.73. Caudate DA accumulation precedes NAcc spike by ~2.1s."
- **CAN mention**: Everything — belief names, indices, relay fields, kernel version, phase schedule

## Level-Gated Observations

Observation types unlock as the user's persona level increases:

| Level | Unlocked Observation Types |
|-------|--------------------------|
| L2+ | mood_landscape |
| L3+ | daily_reflection |
