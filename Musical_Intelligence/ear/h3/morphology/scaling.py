"""Morph normalization dispatch: maps raw morph outputs to [0, 1].
...
def normalize_morph(raw: Tensor, morph_idx: int) -> Tensor:
    """Normalize a raw morph value to [0, 1].
    ...
    Returns:
        Normalized tensor clamped to [0, 1], same shape as *raw*.
    """
    scale = MORPH_SCALE[morph_idx]

    if morph_idx in SIGNED_MORPHS:
        return torch.clamp((raw / scale + 1.0) / 2.0, 0.0, 1.0)
    else:
